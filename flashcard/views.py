...
def edit_set(request, set_id):
    ...
    card_set = Set.objects.get(id=set_id)
    current_cards = card_set.card_set.all()
    card_form_set = formset_factory(CardForm)
    ...
    formset = card_form_set(initial_data)
    title_form = SetForm(initial={'name': card_set.name, 'color': card_set.color})
    # formset.is_valid()
    return render(request, 'flashcard/edit.html', {
        'set_id': set_id, 'title_form': title_form, 'formset': formset,
        'empty_card_index': empty_card_index, 'create': 0, 'color': card_set.color, 'admin': admin
    })
...
def learn(request, set_id, question='front'):
    flashcards_data = Set.objects.get(id=set_id)
    print(flashcards_data.card_set.all())
    flashcards = serializers.serialize("json", flashcards_data.card_set.all())
    return render(request, 'flashcard/learn.html', {
        'title': flashcards_data.name, 'cards': flashcards, 'set_id': set_id, 'admin': admin, 'question': question,
    })

def learn_back(request, set_id):
    return learn(request, set_id, question='back')
